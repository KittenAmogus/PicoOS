...
static FILE *_file_descriptors[MAX_FILE_DESCRIPTORS] = {NULL};
FILE *stdin = NULL;
FILE *stdout = NULL;
FILE *stderr = NULL;
...
int k_register_stream(FILE *stream) {
  if (stream == NULL)
    return EINVAL;
  if (stream->fd >= MAX_FILE_DESCRIPTORS)
    return ENOMEM;
  if (_file_descriptors[stream->fd] != NULL)
    return EEXIST;

  _file_descriptors[stream->fd] = stream;
...
int k_unregister_stream(FILE *stream) {
  if (stream == NULL)
    return EINVAL;
  if (stream->fd >= MAX_FILE_DESCRIPTORS)
    return EINVAL;
  ...
  _file_descriptors[stream->fd] = NULL;
  return SUCCESS;
}
...
static uint32_t div10(uint32_t n) {
  uint64_t q = ((uint64_t)n * 0xCCCCCCCD) >> 35;
  return (uint32_t)q;
}
