...
void k_mem_init(void) {
  ...
  _first_block = (mem_block_t *)&__end__;
  _first_block->magic = MEM_BLOCK_MAGIC;
  _first_block->prev = NULL;
  _first_block->next = NULL;
  _first_block->size = ((size_t)__HeapLimit - (size_t)_first_block) - META_SIZE;
...
void *malloc(size_t size) {
  ...
  while (block != NULL && block->magic == MEM_BLOCK_MAGIC) {
    ...
    if (block->is_free && block->size >= size) {
      ...
      // Not enough RAM
      if ((size_t)block + size >= (size_t)__HeapLimit)
        return NULL;
